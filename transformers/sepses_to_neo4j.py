...
class SEPSesToNeo4jTransformer:
    ...
    def _create_logevent_queries(chain: AttackChain) -> List[str]:
        """Generate CREATE & MERGE queries for LogEvent nodes with relationships"""
        queries = []
        ...
        for event in chain.log_events:
            ...
            queries.append(f"MERGE (e:LogEvent {{ {props_str} }}) SET e:SEPSesLogEvent")
            ...
            # for rel in rels:
            #     queries.append(f"MERGE {rel}")

            rels = []
            if event.host:
                rels.append(f"(:Host {{id: '{event.host.id}'}})|HAS_HOST")
            if event.process:
                rels.append(f"(:Process {{id: '{event.process.id}'}})|HAS_PROCESS")
            if event.user:
                rels.append(f"(:User {{id: '{event.user.id}'}})|HAS_USER")
            if event.file:
                rels.append(f"(:File {{id: '{event.file.id}'}})|HAS_FILE")
            if event.network_flow:
                rels.append(f"(:NetworkFlow {{id: '{event.network_flow.id}'}})|HAS_NETWORK_FLOW")
    ...
    def _create_temporal_queries(chain: AttackChain) -> List[str]:
        """Create & MERGE PRECEDES relationships between consecutive LogEvents"""
        queries = []
    ...
    def _create_chain_metadata_queries(chain: AttackChain) -> List[str]:
        """Create & MERGE AttackChain metadata node with relationships to all events"""
        queries = [

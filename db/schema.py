...
class Neo4jSchemaManager:
    ...
    #     WHERE state = 'ONLINE'
    #     RETURN 'Index' AS category, name AS name, labelsOrTypes[0] AS label, properties[0] AS property, type AS description
    #     UNION ALL
    #     SHOW CONSTRAINTS YIELD name, type, labelsOrTypes, properties
    #     WHERE entityType="NODE"
    #     RETURN 'Constraint' AS category, name AS label, properties[0] AS property, type AS description
    #     """
    @staticmethod
    async def get_schema_info_query(neo4j_client) -> tuple:
        ...
        RETURN 'Constraint' AS category, name AS label, properties[0] AS property, type AS description
        """
        CONSTRAINT_STATUS = await neo4j_client.execute_read(query_constraints)

        return INDEXES_STATUS, CONSTRAINT_STATUS
    ...
    async def check_schema_status(neo4j_client) -> Dict:
        ...
        try:
            INDEXES_STATUS , CONSTARINT_STATUS = await Neo4jSchemaManager.get_schema_info_query(neo4j_client)
            #result = await neo4j_client.execute_read(query)
            
            indexes = [r for r in INDEXES_STATUS if r["category"] == "Index"]
            constraints = [r for r in CONSTARINT_STATUS if r["category"] == "Constraint"]

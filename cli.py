...
@app.command()
def process(
    time_gte: Optional[str] = typer.Option(None, "--from", help="Start time (e.g., 'now-1y')"),
        ...
        async with AsyncDataManager() as manager:
            if action == "status":
                ...
                if result: # Check if result is not None or empty
                    indexes_created = result.get('indexes_created', 'N/A') # Use .get() for safer access
                    print(f"indexes_created:{indexes_created}\n")
                    constraints_created = result.get('constraints_created', 'N/A')
                    print(f"constraints_created:{constraints_created}\n")
                    console.print(Panel.fit(
                        ...
                        console.print(Panel.fit(
                        "[bold yellow]⚠️ Schema initialization may have failed or returned no data.[/bold yellow]",
                        title="Schema Initialization",
                        border_style="yellow"
                    ))    
